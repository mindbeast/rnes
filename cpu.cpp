...
// clear carry flag
void Cpu::clcInst(uint16_t)
{
    clearFlag(CARRY);
}

// clear decimal mode
void Cpu::cldInst(uint16_t)
{
    clearFlag(DECIMAL);
}

// clear interrupt disable flag
void Cpu::cliInst(uint16_t)
{
    clearFlag(INT_DISABLE);
}

// clear OVERFL flag
void Cpu::clvInst(uint16_t)
{
    clearFlag(OVERFL);
}
...
void Cpu::cmpInst(uint16_t addr)
{
    cmp(addr, a);
}

// compare x to memory
void Cpu::cpxInst(uint16_t addr)
{
    cmp(addr, x);
}

// compare y to memory
void Cpu::cpyInst(uint16_t addr)
{
    cmp(addr, y);
}
...
void Cpu::jmpInst(uint16_t addr)
{
    pc = addr;
}
...
void Cpu::phaInst(uint16_t)
{
    pushStack(a);
}

void Cpu::phpInst(uint16_t)
{
...
// set carry flag
void Cpu::secInst(uint16_t)
{
    setFlag(CARRY);
}

// set bcd mode
void Cpu::sedInst(uint16_t)
{
    setFlag(DECIMAL);
}

// set interrupt disable
void Cpu::seiInst(uint16_t)
{
    setFlag(INT_DISABLE);
}

// store accumulator to memory
void Cpu::staInst(uint16_t addr)
{
    store(addr, a);
}

// store x to memory
void Cpu::stxInst(uint16_t addr)
{
    store(addr, x);
}

// store y to memory
void Cpu::styInst(uint16_t addr)
{
    store(addr, y);
}
